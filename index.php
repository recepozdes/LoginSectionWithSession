...
session_start();
    ...
    }  //session başlatılmadı ya da false ise index sayfası bizi login sayfasına yönlendirecek. burada amaç index sayfasının içeriğini görebilmek için bir oturuma bağlı olmamız gerektiğidir
    ...
    <div class="card <?= cookie('color')? cookie('color') : 'bg-dark';?>" style="width: 18rem;">
        ...
        <div class="card-body">
            ...
            <form action="islem.php?islem=hakkimda" method="post">
                <textarea class="form-control <?= cookie('color')? cookie('color') : 'bg-dark';?> text-danger" name="hakkimda" id="" cols="30" rows="10"><?=htmlspecialchars_decode($hakkimda); ?></textarea>
